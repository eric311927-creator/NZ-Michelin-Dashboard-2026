...
function getFilteredData() {
  let data = RESTAURANTS.filter(r => {
    ...
    if (state.budget === 'within' && !r.within) return false;
    if (state.query) {
...
function budgetTagHtml(within) {
  return within
    ? '<span class="platform-pill" style="color:var(--color-primary)">✓ 預算內</span>'
    : '<span class="platform-pill" style="opacity:.7">超出預算</span>';
}
...
function renderTableBody() {
  ...
  tbody.innerHTML = data.map(r => `
    <tr>
      ...
      <td class="price-cell">${r.priceNT === null ? '洽詢' : 'NT$' + r.priceNT.toLocaleString() + '起'}<span class="surcharge">${r.priceDisplay}</span>${budgetTagHtml(r.within)}</td>
      <td>
        <div class="platform-list">${r.reservation ? `<span class="platform-pill">${r.reservation}</span>` : ''}</div>
      </td>
      <td class="signature-cell">${r.signature || '—'}${r.leadTime ? `<div class="rest-zh">${r.leadTime}</div>` : ''}</td>
      <td class="chef-cell">${r.chef || '—'}</td>
...
function openModal(id) {
  ...
  document.getElementById('modalBody').innerHTML = `
    ...
    <dl>
      ...
      <dt>主廚</dt><dd>${r.chef || '未公開'}</dd>
      <dt>狀態</dt><dd>${r.status}</dd>
      <dt>價位</dt><dd>${r.priceNT === null ? '洽詢餐廳' : 'NT$' + r.priceNT.toLocaleString() + '起'} <span style="color:var(--color-text-faint)">${r.priceDisplay}</span></dd>
      <dt>是否在NT$5,000預算內</dt><dd>${r.within ? '是' : '否，可能超出'}</dd>
      <dt>訂位方式</dt><dd>${r.reservation}</dd>
      ${r.leadTime ? `<dt>候補／備註</dt><dd>${r.leadTime}</dd>` : ''}
      ${r.signature ? `<dt>招牌特色</dt><dd>${r.signature}</dd>` : ''}
    </dl>
  `;
...
function renderCharts() {
  ...
  // Budget donut
  const withinCount = RESTAURANTS.filter(r => r.within).length;
  const overCount = RESTAURANTS.length - withinCount;
  if (budgetChart) budgetChart.destroy();
  budgetChart = new Chart(document.getElementById('budgetChart'), {
    type: 'doughnut',
    data: {
      labels: ['NT$5,000以內', '可能超出預算'],
      datasets: [{ data: [withinCount, overCount], backgroundColor: [primary, cssVar('--color-text-faint')], borderWidth: 0 }],
    },
    options: {
      responsive: true, maintainAspectRatio: false,
      cutout: '68%',
...
function renderKpis() {
  ...
  const within = RESTAURANTS.filter(r => r.within).length;
